...
import {
	...
	createProgressLog,
	deleteProgressLog,
} from "@/_actions/projects/progress";
import { getMilestones } from "@/_actions/projects/milestones";
import { getTasks } from "@/_actions/projects/tasks";
...
export function ProgressLogList({ projectId }: ProgressLogListProps) {
	...
	async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
		...
		if (error) {
			toast({
				title: "エラー",
				description: "進捗ログの作成に失敗しました",
				variant: "destructive",
			});
		} else {
			toast({
				title: "成功",
				description: "進捗ログを作成しました",
			});
			setOpen(false);
	...
	async function handleDelete(id: string) {
		...
		if (error) {
			toast({
				title: "エラー",
				description: "進捗ログの削除に失敗しました",
				variant: "destructive",
			});
		} else {
			toast({
				title: "成功",
				description: "進捗ログを削除しました",
			});
			const { data } = await getProgressLogs(projectId);
			if (data) setLogs(data.map(convertProgressLog));
		}
