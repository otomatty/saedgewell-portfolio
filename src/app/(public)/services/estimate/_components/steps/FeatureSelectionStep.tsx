...
export function FeatureSelectionStep() {
	...
	const generateFeaturesList = useCallback(async () => {
		...
		try {
			setGenerationStatus({
				step: 1,
				message: statusMessages[0].loading,
				completed: false,
			});

			const result = await generateFeatures(formData);
			setProposedFeatures(result.features);
			setGenerationStatus(result.status);

			const requiredFeatureIds = result.features
	...
	}, [formData, setProposedFeatures, setSelectedFeatureIds]);
