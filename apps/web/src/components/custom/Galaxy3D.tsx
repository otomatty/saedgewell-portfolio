...
export const Galaxy3D: React.FC<Galaxy3DProps> = ({ className }) => {
	...
	useEffect(() => {
		...
		const handleScroll = () => {
			...
			galaxy.material.uniforms.uSize.value =
				(startSize - (startSize - endSize) * easedProgress) *
				renderer.getPixelRatio();

			// 銀河の回転速度を調整（より緩やかに）
			galaxy.material.uniforms.uTime.value *= 1 - easedProgress * 0.3;
		};
		...
		const handleResize = () => {
			...
			renderer.setSize(window.innerWidth, window.innerHeight);
			renderer.setPixelRatio(Math.min(window.devicePixelRatio, 2));
			galaxy.material.uniforms.uSize.value = 45 * renderer.getPixelRatio();
		};
