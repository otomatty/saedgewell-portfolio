 ...
 * スキルの使用実績一覧を取得する
 */
export async function getSkillExperiences(
	skillId: string,
	...
	if (error) {
		throw new Error("使用実績の取得に失敗しました");
	}

	return experiences as SkillExperience[];
}

/**
 * スキルの使用実績を作成する
 */
export async function createSkillExperience(
	input: CreateSkillExperienceInput,
): Promise<SkillExperience> {
	const supabase = await createClient();
	...
	if (error) {
		throw new Error("使用実績の作成に失敗しました");
	}

	return experience as SkillExperience;
}
...
export async function updateSkillExperience(
	id: string,
	input: UpdateSkillExperienceInput,
): Promise<SkillExperience> {
	const supabase = await createClient();
	const { data: experience, error } = await supabase
		.from("skill_experiences")
	...
	if (error) {
		throw new Error("使用実績の更新に失敗しました");
	}
 ...
 * スキルの使用実績を削除する
 */
export async function deleteSkillExperience(id: string): Promise<void> {
	const supabase = await createClient();
	...
	if (error) {
		throw new Error("使用実績の削除に失敗しました");
	}
}
